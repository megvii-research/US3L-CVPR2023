...
parser.add_argument('--qcl', action='store_true', default=False)

parser.add_argument('--width-mult', default=1.0, type=float, help='width multiplier')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if 'vit' in args.arch:
        ...
        #if args.qcl:
        #    qmodel = quant_tools.QuantModel(model, config)
        #    qmodel.set_quant_state(w_quant=False, a_quant=False, w_init=False, a_init=False)
        linear_keyword = 'head'
    else:
    ...
    train_dataset = cifar10_dataset(transform=train_transforms, mode="train")
    val_dataset = cifar10_dataset(transform=val_transforms, mode="validation")

    #train_dataset = imagenet_dataset(transform=train_transforms, mode="train")
    #val_dataset = imagenet_dataset(transform=val_transforms, mode="validation")
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            ...
            #if args.qcl:
            #    state_dict = {k[len('model.'):]: state_dict[k] for k in list(state_dict.keys()) if k.startswith('model.')}
